	...
	while (!dexit) {
				...
				while (true) {
					...
					if (keybuf[0] == 'm') {
						...
						for (int w = 0; w < cnt; w++) {
							...
							for (int i = 0; i < 4; i++) { printf("%02x ", mem[addr + (w * 4) + i]); }
							printf("\n");
						}
					}
					if (keybuf[0] == 'l') {
						...
						for (int i = 0; i < DATA_OFFSET / 4; i++) {
							instr* ii = &imem[i];
							if (ii->orig_line >= 0 && ii->psrc) { printf("%9d: %s\n", ii->orig_line, ii->psrc); }
						}
					}
				}
			}
		...
		switch (i.op) {
			...
			case JALR:
				...
				break;
			case AUIPC: rf[i.a1.reg] = pc + (i.a2.imm << 12); break;
			case LUI: rf[i.a1.reg] = (i.a2.imm << 12); break;
...
int main(int argc, char** argv) {
	...
	if (argc >= 3) { start_immediate = true; }
